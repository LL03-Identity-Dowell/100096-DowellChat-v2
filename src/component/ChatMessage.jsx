  ...
  return (
    <div className={`${message?.isSender ? "self-end" : "self-start"}`}>
      ...
      <div
        className={`${
          message?.isSender
            ? "bg-green-300 rounded-full rounded-tr"
            : "bg-blue-300 rounded-full rounded-tl self-start"
        } flex items-center text-sm sm:text-base text-black/50 shadow-lg py-2 px-3`}
      >
        <IoIosCheckmark className="text-2xl" />
        <p>{message?.content}</p>
      </div>
