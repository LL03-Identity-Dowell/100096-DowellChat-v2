                ...
                if msg.error():
                    ...
                    if check_daily_collection(api_key, workspace_id, product):
                        ...
                        response = data_cube.insert_data(
                            api_key=api_key, db_name=db_name, coll_name=coll_name, data=message)

                        if response['success'] == True:
                            sio.emit('ticket_message_response', {
                                     'data': "Message Send to DB", 'status': 'success', 'operation': 'send_message'}, room=sid)
                            print(response)
                            # return
